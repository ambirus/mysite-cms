        ...
        <div id="flags">
            <ul>
                <?php foreach ($langs as $k => $v) : ?>
                    <li>
                        <a href="?lang=<?=$k?>"><img src="/images/flags/<?=$v['icon']?>" alt="<?=$v['title']?>" <?=($currLang == $k ? 'class="curr-lang"' : '')?>></a>
                    </li>
                <?php endforeach; ?>
            </ul>
        </div>
        <div>
            ...
            <form action="/" method="post">
                <table align="center">
                    ...
                    <tr>
                        <td></td>
                        <td><input style="width: 13px;" type="checkbox" name="Install[dbcurrent]" value="1"> <small>{{%check this option if you're not allowed to create a new database%}}</small></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="Install[siteTheme]">
                                <option <?=$values['siteTheme'] == 'blue' ? 'selected' : ''?> value="blue">{{%default (blue)%}}</option>
                                <option <?=$values['siteTheme'] == 'green' ? 'selected' : ''?> value="green">{{%green%}}</option>
                                <option <?=$values['siteTheme'] == 'red' ? 'selected' : ''?> value="red">{{%orange%}}</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <select name="Install[siteLanguage]">
                                <?php foreach ($langs as $k => $lang) : ?>
                                    <option <?=$values['siteLanguage'] == $k ? 'selected' : ''?> value="<?=$k?>"><?=$lang['title']?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                    </tr>
